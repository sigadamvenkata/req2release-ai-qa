...
- **Integrations:** Which external services (Slack, jira etc.) do we need? Are keys ready?
...
**2. Data-First Rule:** You must define the **JSON Data Schema** (Input/Output shapes) in `claude.md`. Coding only begins once the "Payload" shape is confirmed.
...
**1. Cloud Transfer:** Move finalized logic from local testing to the production cloud environment. **2. Automation:** Set up execution triggers (Cron jobs, Webhooks, or Listeners). **3. Documentation:** Finalize the **Maintenance Log** in `claude.md` for long-term stability.
